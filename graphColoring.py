...
def crossover2(parent, otherParent):
    childGenes = parent[:]
...
def crossover(parent, otherParent):
    childGenes = otherParent[:]
    if len(parent) <= 2 or len(otherParent) < 2:
        return childGenes
    length = random.randint(1, len(parent) - 2)
    start = random.randrange(0, len(parent) - length)
    childGenes[start:start + length] = parent[start:start + length]
    for i in range(start, start + length):
        if otherParent[i] not in childGenes[start:start + length]:
            j = i
            initialValue = otherParent[j]
            while parent[j] in otherParent[start:start + length] and j in range(start, start + length):
                j = otherParent.index(parent[j])
            childGenes[otherParent.index(parent[j])] = initialValue
    return childGenes
...
class GraphColoringTests(unittest.TestCase):
    ...
    def color(self, file, colors):
        ...
        best = genetic.get_best(fnGetFitness, len(nodes), optimalValue, geneset, fnDisplay, custom_mutate=fnMutate,
                                maxAge=500, poolSize=25, crossover=fnCrossover)
